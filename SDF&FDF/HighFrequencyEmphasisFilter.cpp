...
int main()
{
	...
	Mat high_filter = get_highpassFilter(dft_coef.size(), 40);  // cut-off 70 고주파 필터 생성
	...
	Mat inv_img = IFFT(filtered_mat, image.size());

	for (int i = 0; i < image.rows; i++) {
		for (int j = 0; j < image.cols; j++) {
			HPE_img.at<uchar>(i, j) = image.at<char>(i, j) + inv_img.at<uchar>(i, j);
		}
	}
	imshow("HFEF 적용 영상", HPE_img);
	waitKey();

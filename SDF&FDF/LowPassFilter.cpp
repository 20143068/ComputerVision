...
int main()
{
	...
	Mat low_filter = get_lowpassFilter(dft_coef.size(), 70);  // 저주파 필터 생성

	multiply(dft_coef, low_filter, filtered_mat);  //원소간 곱셈
	log_mag(filtered_mat, low_dft);

	imshow("원본 영상", image);
	imshow("FFT + log scale + shuffling 적용 영상", dft_img);
	imshow("Low Pass Filter 적용 영상", low_dft);  // 필터된 스펙트럼 영상	
	imshow("Inverse FFT 적용 후 복원된 영상", IFFT(filtered_mat, image.size()));	// 스펙트럼 영상 복원
	waitKey();
	return 0;
}